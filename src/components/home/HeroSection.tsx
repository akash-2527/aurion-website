...
import { ScrollTrigger } from "gsap/ScrollTrigger";
import heroBg from "@/assets/hero-bg.jpg";
import teamPhoto from "@/assets/hero-team.webp";
import HoverAnimator from "@/motion/HoverAnimator";
...
// ─── Word-by-word reveal ──────────────────────────────────────────────────────
//
// FIX: The crash originates here.
//
// Root cause: gsap.set(words, { filter: "blur(5px)" }) then
// gsap.to(words, { filter: "blur(0px)" }) causes GSAP's CSSPlugin to call
// _tweenComplexCSSString2 on the filter value. That function calls
// _colorStringFilter2 → splitColor2(str) where str.match(colorExp) returns
// null for the "blur()" string (no colour tokens). splitColor2 then tries to
// call .map() on null → TypeError crash.
//
// Fix: remove filter entirely from GSAP. Instead, set filter via inline style
// on each word span and rely on a CSS transition (blur 0.5s) to clear it as
// GSAP's opacity/y tween runs. GSAP never owns the filter property → zero
// conflict. Visual result is identical: words appear with a dissolving blur.
// ─────────────────────────────────────────────────────────────────────────────
function WordReveal({
  text,
  className,
  ...
  useEffect(() => {
    ...
    // GSAP owns only opacity and y — no filter property here
    ...
    const ctx = gsap.context(() => {
      gsap.to(words, {
        ...
        stagger,
        delay: baseDelay,
        // When each word's opacity tween starts, its inline style switches
        // filter to blur(0px); the CSS transition on the span handles the
        // smooth clear — GSAP never parses the filter string.
        onStart() {
          // noop — stagger means each word triggers its own onStart;
          // the blur clear is driven purely by the CSS transition below.
        },
      });
    }, container);

    // Kick the blur clear: after the tween starts the CSS transition takes over.
    // We schedule per-word via setTimeout mirroring the stagger timing so each
    // word's blur dissolves in sync with its opacity/y reveal.
    words.forEach((word, i) => {
      const delay = (baseDelay + i * stagger) * 1000;
      const id = window.setTimeout(() => {
        word.style.filter = "blur(0px)";
      }, delay);
      // Store id on the element so cleanup can cancel if needed
      (word as HTMLElement & { _blurTimer?: number })._blurTimer = id;
    });

    return () => {
      ctx.revert();
      words.forEach(word => {
        const id = (word as HTMLElement & { _blurTimer?: number })._blurTimer;
      ...
      {text.split(" ").map((word, i) => (
        <span
          ...
          style={{
            marginRight: "0.26em",
            opacity: 0,
            // Start blurred; CSS transition smoothly clears it when the
            // setTimeout above sets filter: blur(0px)
            filter: "blur(5px)",
...
// ─── HeroImage — geometric reveal + parallax ─────────────────────────────────
//
// Three refs, three owners, zero transform conflict:
//   wrapRef       — clip-path only  (GSAP entrance)
//   mouseLayerRef — translate only  (direct RAF style — GSAP never touches)
//   imgRef        — scale + y only  (GSAP entrance + scroll scrub)
// ─────────────────────────────────────────────────────────────────────────────
function HeroImage() {
  const wrapRef       = useRef<HTMLDivElement>(null);
  ...
  // ── Scroll parallax: GSAP animates y on imgRef only ─────────────────────
  useEffect(() => {
    const img  = imgRef.current;
    const wrap = wrapRef.current;
    if (!img || !wrap) return;
  ...
  // ── Mouse parallax: translate written to mouseLayerRef ONLY ─────────────
  // imgRef.style.transform is never touched here.
  useEffect(() => {
    ...
    const tick = () => {
      cx += (tx - cx) * 0.055;
      cy += (ty - cy) * 0.055;
      mouseLayer.style.transform = `translate(${cx}px, ${cy}px)`;
      rafId = requestAnimationFrame(tick);
    };

...
function PillarCard({ pillar, scrollDelay }: { pillar: typeof pillarsData[0]; scrollDelay: number }) {
  ...
  // FIXED: Simplified animation to avoid GSAP color parsing
  useEffect(() => {
    ...
    try {
      // Only animate opacity and y, no 3D transforms that might confuse GSAP
      gsap.set(el, { 
        opacity: 0, 
        y: 42,
        force3D: true 
    ...
    return () => {
      if (scrollTrigger) {
        try { scrollTrigger.kill(); } catch (e) {}
      }
      if (ctx) {
    ...
    <div ref={outerRef} style={{ opacity: 0, willChange: "transform, opacity" }}>
      <div ref={innerRef} style={{ transformStyle: "preserve-3d", willChange: "transform", height: "100%" }}>
        <div 
          ...
          onMouseEnter={(e) => { 
            e.currentTarget.style.boxShadow = "0 18px 44px -8px rgba(0,0,0,0.11), 0 5px 16px -4px rgba(0,0,0,0.07)"; 
        ...
        >
          ...
          <div className="p-8 md:p-10">
            <span 
              className="font-body font-semibold block mb-6" 
              style={{ 
                fontSize: "11px", 
